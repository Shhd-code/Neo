...
-- نسخ سبام مع زر الإيقاف
local spamRowA = Instance.new("Frame", copyPage)
spamRowA.BackgroundTransparency = 1
spamRowA.Position = UDim2.new(0, 10, 0, 130); spamRowA.Size = UDim2.new(1, -20, 0, 46)
local copySpamBtn = makeBigBtn(spamRowA, "نسخ سبام", 0)
copySpamBtn.Size = UDim2.new(0.5, -6, 1, 0)
local stopBtnA = makeBigBtn(spamRowA, "إيقاف", 0, Color3.fromRGB(0, 180, 80), Color3.fromRGB(0, 120, 50))
stopBtnA.Position = UDim2.new(0.5, 4, 0, 0); stopBtnA.Size = UDim2.new(0.5, -6, 1, 0)

-- نسخ logs مع زر الإيقاف
local spamRowB = Instance.new("Frame", copyPage)
spamRowB.BackgroundTransparency = 1
spamRowB.Position = UDim2.new(0, 10, 0, 184); spamRowB.Size = UDim2.new(1, -20, 0, 46)
local copyLogsBtn = makeBigBtn(spamRowB, "نسخ logs", 0, Color3.fromRGB(220, 140, 30), Color3.fromRGB(160, 90, 10))
copyLogsBtn.Size = UDim2.new(0.5, -6, 1, 0)
local stopBtnB = makeBigBtn(spamRowB, "إيقاف", 0, Color3.fromRGB(0, 180, 80), Color3.fromRGB(0, 120, 50))
stopBtnB.Position = UDim2.new(0.5, 4, 0, 0); stopBtnB.Size = UDim2.new(0.5, -6, 1, 0)

-- نسخ re مع زر الإيقاف
local spamRowC = Instance.new("Frame", copyPage)
spamRowC.BackgroundTransparency = 1
spamRowC.Position = UDim2.new(0, 10, 0, 238); spamRowC.Size = UDim2.new(1, -20, 0, 46)
local copyReBtn = makeBigBtn(spamRowC, "نسخ re", 0, Color3.fromRGB(255, 100, 150), Color3.fromRGB(200, 50, 100))
copyReBtn.Size = UDim2.new(0.5, -6, 1, 0)
local stopBtnC = makeBigBtn(spamRowC, "إيقاف", 0, Color3.fromRGB(0, 180, 80), Color3.fromRGB(0, 120, 50))
stopBtnC.Position = UDim2.new(0.5, 4, 0, 0); stopBtnC.Size = UDim2.new(0.5, -6, 1, 0)
...
copySpamBtn.MouseButton1Click:Connect(function()
    if spamARunning then
        ...
        local msg = buildSpamA(selectedName, prefix)
        startSpam(msg)
        spamARunning = true
        copySpamBtn.Text = "إيقاف"
    end
end)

-- معالج إيقاف السبام A
stopBtnA.MouseButton1Click:Connect(function()
    if spamARunning then
        stopSpam()
        spamARunning = false
        TweenService:Create(stopBtnA, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(0, 180, 80)}):Play()
    else
        if not selectedName then setStatus("اختر لاعب اولا") return end
        if checkBlockedPlayer(selectedName) then setStatus("ممنوع تنسخ صاحبة السكربت ✋😜") return end
        local prefix = (prefixBox.Text ~= "" and prefixBox.Text) or ";"
        ...
        startSpam(msg)
        spamARunning = true
        TweenService:Create(stopBtnA, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(200, 30, 30)}):Play()
    end
end)

-- معالج نسخ logs (toggle)
local logsSpamRunning = false
copyLogsBtn.MouseButton1Click:Connect(function()
    if logsSpamRunning then
        stopSpam()
        logsSpamRunning = false
        copyLogsBtn.Text = "نسخ logs"
    else
        if not selectedName then setStatus("اختر لاعب اولا") return end
        if checkBlockedPlayer(selectedName) then setStatus("ممنوع تنسخ صاحبة السكربت ✋😜") return end
        local prefix = (prefixBox.Text ~= "" and prefixBox.Text) or ";"
        local msg = buildLogsCmd(selectedName, prefix)
        startSpam(msg)
        logsSpamRunning = true
        copyLogsBtn.Text = "إيقاف"
    end
end)
...
stopBtnB.MouseButton1Click:Connect(function()
    if logsSpamRunning then
        stopSpam()
        logsSpamRunning = false
        TweenService:Create(stopBtnB, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(0, 180, 80)}):Play()
    else
        if not selectedName then setStatus("اختر لاعب اولا") return end
        if checkBlockedPlayer(selectedName) then setStatus("ممنوع تنسخ صاحبة السكربت ✋😜") return end
        local prefix = (prefixBox.Text ~= "" and prefixBox.Text) or ";"
        ...
        startSpam(msg)
        logsSpamRunning = true
        TweenService:Create(stopBtnB, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(200, 30, 30)}):Play()
    end
end)
...
copyReBtn.MouseButton1Click:Connect(function()
    if reSpamRunning then
        stopSpam()
        reSpamRunning = false
        copyReBtn.Text = "نسخ re"
    else
        if not selectedName then setStatus("اختر لاعب اولا") return end
        if checkBlockedPlayer(selectedName) then setStatus("ممنوع تنسخ صاحبة السكربت ✋😜") return end
        local prefix = (prefixBox.Text ~= "" and prefixBox.Text) or ";"
        local msg = buildReCmd(selectedName, prefix)
        startSpam(msg)
        reSpamRunning = true
        copyReBtn.Text = "إيقاف"
    end
end)
...
stopBtnC.MouseButton1Click:Connect(function()
    if reSpamRunning then
        stopSpam()
        reSpamRunning = false
        TweenService:Create(stopBtnC, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(0, 180, 80)}):Play()
    else
        if not selectedName then setStatus("اختر لاعب اولا") return end
        if checkBlockedPlayer(selectedName) then setStatus("ممنوع تنسخ صاحبة السكربت ✋😜") return end
        local prefix = (prefixBox.Text ~= "" and prefixBox.Text) or ";"
        ...
        startSpam(msg)
        reSpamRunning = true
        TweenService:Create(stopBtnC, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(200, 30, 30)}):Play()
    end
end)
